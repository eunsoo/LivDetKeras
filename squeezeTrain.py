...
#%%
    ...
    parser.add_option("-p", "--pretmorain", dest="preTrain", default="dd", 
                      help="Pretraining Model")
    parser.add_option("-b", "--binary", dest="binary", action="store_false",
                      default=True, help="Binary Class?")
    parser.add_option("-r", "--resultFolder", dest="result",
                      default="models/final", help="saving folder")
    parser.add_option("-e", "--epoch", dest="nb_epoch", default=20, help="Number of Epoch")
    ...
    parser.add_option("-t", "--active", dest="active", action="store_true", default = False, help="only Active area?")
    parser.add_option("-l", "--lrate", dest="lr", default=0.002, help="set learing rate.")
    parser.add_option("-i", "--init", dest="init", action="store_true", default=False, help="Raw model(not pretrained.)")
    ...
    if active:
      tempFolder = os.path.join(dataHome,'activeArea', 'Train', sensor, category)
      trainFolder = os.path.expanduser(os.path.join('~', tempFolder))
      tempFolder = os.path.join(dataHome,'activeArea', 'Val', sensor, category)
    ...
    if preTrain == "sz":
        ...
        if not init:
          model.load_weights(basemodel,by_name=True)
    elif preTrain == "sx":
        ...
        model = sq.get_sqAllXceNet(classNum, dim_ordering='tf')
        if not init:
          model.load_weights(basemodel, by_name=True)
    elif preTrain == "sb":
        model = sq.get_sqAllXceNetSr50(classNum, dim_ordering='tf')
        if not init:
          model.load_weights(basemodel, by_name=True)
    elif preTrain == "sd":
        model = sq.get_sqAllXceNet_2(classNum, dim_ordering='tf')
        if not init:
          model.load_weights(basemodel, by_name=True)
    elif preTrain == "sr":
        model = sq.get_sqAllXResNet_2(classNum, dim_ordering='tf')
        if not init:
          model.load_weights(basemodel, by_name=True)
    elif preTrain =="sw":
        model = sq.get_sqAllXceNetFinalWide(classNum, dim_ordering='tf')
        if not init:
          model.load_weights(basemodel, by_name=True)
    elif preTrain =="dd":
        model = sq.get_sqAllXceNetFinalDeep(classNum, dim_ordering='tf')
        if not init:
          model.load_weights(basemodel, by_name=True)
    elif preTrain =="gg":
        model = sq.get_sqAllXceNet_depth(classNum, dim_ordering='tf')
        if not init:
          model.load_weights(basemodel, by_name=True)
...
#    #%%    
    ...
    saveFileName = preTrain+prefix+year+'_'+sen+'_'+category
